"""AI-powered YMYL research using ymyl-research-prompt + Kie/Perplexity API."""
...
DEFAULT_PROMPT = ROOT / "prompts" / "ymyl-research-prompt.md"
TIMEOUT_SEC = 240


def _ymyl_ai_config() -> dict[str, Any]:
    cfg = load_config()
    return (cfg.get("ymyl") or {}).get("ai_research") or {}


def load_system_prompt(prompt_path: Path | None = None) -> str:
    path = prompt_path or Path(_ymyl_ai_config().get("prompt_file") or DEFAULT_PROMPT)
...
def _is_privacy_pair(url: str, pair: str | None = None) -> bool:
    blob = f"{url} {pair or ''}".lower()
    return any(c in blob for c in ("xmr", "zec", "dash", "monero", "zcash"))


def build_ymyl_user_message(audit_payload: dict[str, Any]) -> str:
    ai_cfg = _ymyl_ai_config()
    ...
    page_url = audit_payload.get("url") or ""
    pair = audit_payload.get("pair") or ""
    privacy_pair = ai_cfg.get("privacy_pair")
    if privacy_pair is None:
        privacy_pair = _is_privacy_pair(page_url, pair)
    ...
    lines = [
        ...
        f"page_url: {page_url}",
        f"privacy_pair: {str(privacy_pair).lower()}",
        f"mode: {mode}",
        ...
        json.dumps(
            {
                ...
                "pair": pair,
                "page_type": audit_payload.get("page_type"),
            },
            ensure_ascii=False,
            indent=2,
        )[:12000],
    ...
    if ingest_existing:
        lines.append("")
        lines.append("YMYL_LIBRARY_SNAPSHOT (уже в базе — НЕ дублируй, только NEW):")
        lines.append(
            json.dumps(
                {
    ...
    corpus = audit_payload.get("site_corpus")
    if corpus:
        lines.append("")
    ...
    lines.append("")
    lines.append(
        "Начни с Executive Summary, Harm Scenarios table, JSON Document Registry, Criteria Audit (YY01–YY28)."
    )
    return "\n".join(lines)


async def run_ai_ymyl_research(audit_payload: dict[str, Any]) -> dict[str, Any]:
    """Run deep YMYL harm-prevention research via configured model."""
    ai_cfg = _ymyl_ai_config()
    if not ai_cfg.get("enabled"):
        return {"status": "skip", "error": "ymyl.ai_research disabled", "review": None}
    ...
    page_url = audit_payload.get("url") or ""
    page_label = f"YMYL: {audit_payload.get('label') or page_url}"
    run_id: int | None = None
    if persist:
        run_id = start_eeat_research_run(page_url, page_label)

    user_message = build_ymyl_user_message(audit_payload)
